/**
 * ContainsOperator - Filters features that contain a reference geometry
 */
...
export class ContainsOperator extends SpatialOperator {
  readonly operatorId = 'contains';
  readonly name = 'Contains Filter';
  readonly description = 'Filter features that spatially contain a reference data source';
  readonly category = 'analysis' as const;
  readonly returnType = 'spatial' as const;
  readonly inputSchema = ContainsInputSchema;
  ...
  protected async executeCore(params: z.infer<typeof ContainsInputSchema>, context: OperatorContext): Promise<z.infer<typeof SpatialOutputSchema>> {
    ...
    const filterCondition = { type: 'spatial', operation: 'contains', referenceDataSourceId: params.referenceDataSourceId };
    const result = await dataAccess.filter(dataSource.type, dataSource.reference, filterCondition as any);
    const persistedResult = await resultPersistence.persistResult(result, 'contains', dataSource, { referenceDataSourceId: params.referenceDataSourceId });
    return { id: persistedResult.id, type: persistedResult.type, reference: persistedResult.reference, metadata: { ...persistedResult.metadata, operation: 'contains', filteredCount: (persistedResult as any).features?.length || 0 } };
  }
}
