/**
 * Style Factory - Universal Map Visualization Style Generator (Frontend Version)
 * 
 * Generates Mapbox GL JS compatible style JSON for various visualization types.
 * This is a reusable utility that can be used by any visualization component.
 */
...
export interface ChoroplethStyleConfig {
  ...
  valueField: string
  breaks: number[]
  colorRamp: string           // Color ramp name
  numClasses: number
  opacity?: number
  geometryType?: GeometryType
  minZoom?: number
...
export class StyleFactory {
   ...
   */
  static async generateUniformStyle(config: UniformStyleConfig): Promise<MapboxStyle> {
    const {
      tilesetId,
      layerName,
      color,
   ...
   */
  static async generateCategoricalStyle(config: CategoricalStyleConfig): Promise<MapboxStyle> {
    const {
      tilesetId,
      layerName,
      categoryField,
  ...
  /**
   * Generate choropleth style
   */
  static async generateChoroplethStyle(config: ChoroplethStyleConfig): Promise<MapboxStyle> {
    const {
      tilesetId,
      layerName,
      valueField,
